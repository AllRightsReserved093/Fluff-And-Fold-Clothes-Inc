...
def create_machines(http_client: httpx.Client) -> dict[str, Machine]:
...
def parse_console_command(command_text: str, selected_machine_id: str | None, machine_ids: set[str]) -> tuple[Command | None, str | None]:
    ...
    if action == "fault":
        if len(parts) != 2:
            print("Usage: fault blocked-vent")
            return None, selected_machine_id
...
def main() -> None:
    ...
    machines = create_machines(http_client)
